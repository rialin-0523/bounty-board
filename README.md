...
主分支最新代码已经在用一套独立的用户/权限库：`users`、`settings`、`created_by`、黑名单和最低斗鱼等级配置。

本次新增的斗鱼绑定/登录库不要去覆盖这套旧用户库，而是并行保留：

- 主分支旧用户系统：`users` / `settings`，负责任务发布权限、黑名单、最低等级控制。
- 本次绑定登录系统：`app_users` / `auth_sessions` / `bind_sessions` / `douyu_profiles`，负责斗鱼 UID 绑定、账号注册和长期登录。

以后合并时，重点是让“旧用户库”继续服务主分支已有功能，同时保留“新绑定库”用于斗鱼账号绑定。
