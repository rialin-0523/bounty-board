  ...
  return (
    <Layout>
      ...
      <div className="home-toolbar">
        ...
        <select className="home-select" value={statusFilter} onChange={e => setStatusFilter(e.target.value)}><option value="全部">全部状态</option><option value="pending">待审核</option><option value="rejected">已拒绝</option><option value="active">进行中</option><option value="expired">已到期</option><option value="completed">已完成</option><option value="cancelled">已取消</option></select>
      ...
      {loading ? <div className="home-loading">加载中...</div> : display.length === 0 ? <div className="home-empty"><div className="home-empty-icon">⚔️</div><div className="home-empty-text">{challenges.length === 0 ? '暂无任务' : '没有匹配的任务'}</div>{challenges.length === 0 && <Link to="/publish" className="home-empty-cta">+ 发布第一个挑战</Link>}</div> : <div className="cb-list">
        {display.map(c => {
          ...
          return <Link key={c.id} to={`/challenges/${c.id}`} className={`cb-card ${status === 'completed' || status === 'expired' ? 'is-completed' : ''}`}><div className="cb-card-inner"><div className="cb-card-border"></div>
            ...
            <div className={`cb-expiry ${status === 'expired' ? 'is-expired' : ''}`}>{status === 'pending' ? '🕒 正在等待后台审核，通过后才会公开显示' : status === 'rejected' ? `❌ 审核未通过：${c.review_reason || '请修改后重新提交'}` : status === 'active' ? `⏳ ${formatRemainingTime(c.expires_at, now)} · ${c.validity_hours || 3}小时有效` : status === 'expired' ? '⌛ 任务已到期，不能再跟单' : `有效期：${c.validity_hours || 3}小时`}</div>
